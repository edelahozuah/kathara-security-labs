...
BASE_TEST_IP="${BASE_TEST_IP:-192.168.0.2}"
DESKTOP_TEST_IP="${DESKTOP_TEST_IP:-192.168.0.5}"
ROUTER_TEST_IP="${ROUTER_TEST_IP:-192.168.0.1}"
WAN_TEST_IP="${WAN_TEST_IP:-10.255.0.1}"
...
Variables opcionales:
  BASE_TEST_IP              IP del nodo base para pruebas (default 192.168.0.2)
  DESKTOP_TEST_IP           IP del desktop para pruebas (default 192.168.0.5)
  ROUTER_TEST_IP            IP LAN del router (default 192.168.0.1)
  WAN_TEST_IP               IP WAN gateway (default 10.255.0.1)
EOF
}
...
echo "=== Verificacion plantilla escenario base ==="
...
kexec base "ping -c 2 ${ROUTER_TEST_IP}"
if [[ ${DESKTOP_RUNNING} -eq 1 ]]; then
  kexec base "ping -c 2 ${DESKTOP_TEST_IP}"
  kexec desktop "ping -c 2 ${BASE_TEST_IP}"
fi
echo ""

echo "4) Salida hacia gateway WAN"
kexec base "ping -c 2 ${WAN_TEST_IP}"
echo ""

echo "5) Prueba opcional de Internet"
if kexec base "ping -c 2 1.1.1.1"; then
  echo "Internet OK"
else
...
if [[ ${DESKTOP_RUNNING} -eq 1 ]]; then
  echo "6) Verificando desktop (GUI + VNC)"
...
if [[ ${VPN_RUNNING} -eq 1 ]]; then
  ...
  if [[ ${SKIP_HOST_PING} -eq 0 ]]; then
    ...
    ping -c 2 "${BASE_TEST_IP}"
    if [[ ${DESKTOP_RUNNING} -eq 1 ]]; then
      ping -c 2 "${DESKTOP_TEST_IP}"
    else
      ping -c 2 "${ROUTER_TEST_IP}"
    fi
  else
