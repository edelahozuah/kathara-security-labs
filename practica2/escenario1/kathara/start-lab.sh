...
Opciones:
  --endpoint-host <host>  Host/IP para Endpoint en el fichero cliente (por defecto 127.0.0.1)
  --force-proxy           Fuerza el uso del proxy UDP local
  --no-proxy              Desactiva el proxy UDP local
  --wait-seconds <n>      Segundos maximos para esperar a que vpn arranque (por defecto 30)
...
echo "== Inicio escenario Kathara (cross-platform) =="
PLATFORM="$(detect_platform)"
echo "Plataforma detectada: ${PLATFORM}"

if kathara exec -d "${LAB_DIR}" vpn "true" >/dev/null 2>&1; then
  echo "El escenario ya estaba arrancado; se reutiliza."
else
  echo "Arrancando escenario con kathara lstart..."
  kathara lstart -d "${LAB_DIR}"
fi
...
echo "Modo de conectividad: ${MODE_LABEL}"
...
echo "3) Verifica con: ./verify.sh --wait-for-handshake 30"
